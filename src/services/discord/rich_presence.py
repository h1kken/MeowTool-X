...
from src.services.discord.types import PresencePayload, PresenceState, RpcClient
from src.utils.logging import logger


class _DiscordPresenceFallbackError(Exception):
    pass
...
try:
    ...
    Presence = None
    DiscordError = DiscordNotFound = InvalidPipe = PipeClosed = _DiscordPresenceFallbackError
    ActivityType = None
...
class DiscordPresenceManager(QObject):
    ...
    def _snapshot_state(self) -> PresenceState:
        with self._state_lock:
            state = {
    ...
    def _presence_details(self, state: PresenceState) -> str:
        override = state.get('details_override')
        if isinstance(override, str) and override.strip():
            return override.strip()
        return self._normalize_page(str(state.get('page', self.DEFAULT_PAGE)))

    def _presence_state(self, state: PresenceState) -> str:
        override = state.get('state_override')
        if isinstance(override, str) and override.strip():
            return override.strip()
        return ''

    def _build_presence_payload(self, state: PresenceState) -> PresencePayload:
    ...
    @staticmethod
    def _payload_signature(payload: PresencePayload) -> tuple[tuple[str, str], ...]:
        return tuple((str(key), repr(payload[key])) for key in sorted(payload))

    def _disconnect_rpc(self, rpc: RpcClient | None) -> None:
        if rpc is None:
            return
        try:
    ...
    def _cleanup_failed_rpc(self, rpc: RpcClient | None) -> None:
        if rpc is None:
            return
        try:
            rpc.close()
    ...
    def _connect_rpc(
        ...
        client_id: str,
        *,
        missing_library_logged: bool,
        missing_app_id_logged: bool,
    ) -> tuple[RpcClient | None, bool, bool, float]:
        if not client_id:
        ...
        last_pipe_error: str | None = None
        for pipe in pipe_order:
            rpc: RpcClient | None = None
            try:
                rpc = cast(RpcClient, Presence(client_id, pipe=pipe))
                rpc.connect()
                ...
                exc_obj = cast(object, exc)
                if getattr(exc_obj, 'code', None) == 1006 or 'Server at capacity' in str(exc_obj):
                ...
                last_pipe_error = f'DiscordError: {exc_obj}'
                continue
            except (DiscordNotFound, InvalidPipe, OSError):
                self._cleanup_failed_rpc(rpc)
                last_pipe_error = f'Pipe connect skipped on pipe {pipe}'
                continue
        ...
        if last_pipe_error is not None:
            logger.debug(f'Discord Presence connect skipped: {last_pipe_error}')
        return None, False, False, self.RETRY_INTERVAL_SECONDS

    def _publish_presence(self, rpc: RpcClient, payload: PresencePayload) -> bool:
        try:
            rpc.update(**payload)
            return True
