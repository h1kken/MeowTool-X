...
def clone_gradient_data(data: Any) -> dict[str, Any] | None:
...
def adjust_qcolor(color: QColor, *, brightness: float = 1.0, saturation: float = 1.0) -> QColor:
    ...
    if saturation < 0.999:
        gray = round((result.red() * 0.299) + (result.green() * 0.587) + (result.blue() * 0.114))
        result = QColor(
...
def adjust_gradient_data(data: Any, *, brightness: float = 1.0, saturation: float = 1.0) -> dict[str, Any] | None:
...
def parse_gradient_stops(data: Any) -> list[tuple[float, QColor]]:
    ...
    for stop in data:
        ...
        if isinstance(stop, dict):
            ...
            color_value = stop.get('color')
        elif isinstance(stop, (list, tuple)) and len(stop) >= 2:
            pos, color_value = stop[0:2]
        else:
            continue
...
def normalize_gradient_data(data: Any) -> dict[str, Any] | None:
    if not isinstance(data, dict):
        return None

    stops = parse_gradient_stops(data.get('stops'))
    if not stops:
        return None
...
def build_gradient_qss(data: Any) -> str | None:
    ...
    direction = str(gradient.get('direction', 'vertical')).strip().lower() or 'vertical'
...
def build_background_qss_rule(data: Any) -> str | None:
    ...
    if isinstance(gradient, dict):
        if (gradient_qss := build_gradient_qss(gradient)):
...
def build_background_brush(rect: QRectF, data: Any) -> QBrush | None:
    ...
    gradient_source = data.get('gradient') if isinstance(data.get('gradient'), dict) else None
    if gradient_source is None and isinstance(data.get('color'), dict):
        gradient_source = data.get('color')

    if isinstance(gradient_source, dict):
        if (brush := build_gradient_brush(rect, gradient_source)) is not None:
            return brush
...
def build_gradient_brush(rect: QRectF, data: Any) -> QBrush | None:
    ...
    for pos, color_value in gradient.get('stops', []):
        color = to_qcolor(color_value)
        if color is None:
            continue
        brush_gradient.setColorAt(max(0.0, min(1.0, float(pos))), color)
