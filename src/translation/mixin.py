...
class TranslatableMixin:
    def __init__(self, key: str, *args: object, **kwargs: object) -> None:
        ...
        self._key = key
        get_translator().language_changed.connect(self._update_text)
        self._update_text()

    def _update_text(self) -> None:
        cast(_TextWidget, self).setText(get_translator().tr(self._key))
        

class TranslatableComboBoxMixin:
    def __init__(self, *args: object, **kwargs: object) -> None:
        super().__init__(*args, **kwargs)
        get_translator().language_changed.connect(self._update_items_text)

    def add_item(self, item: str) -> None:
        combo = cast(_ComboBoxWidget, self)
        combo.addItem(get_translator().tr(item), item)
        index = combo.count() - 1
    ...
    def _update_items_text(self) -> None:
        ...
        for i in range(combo.count()):
            ...
            if not isinstance(key, str):
                continue
            combo.setItemText(i, get_translator().tr(key))
